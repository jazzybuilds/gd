...
var __1 = require("..");
var __2 = require("..");
function createRendering(r, placeholderKey, index, components, props, logger) {
    if (!r)
    ...
    if (isNodeJs) {
        ...
        if (components[componentLoaderName]) {
            ...
            if (r.componentName !== __1.GLOBAL_LOADER && !componentName.endsWith(__1.COMPONENT_LOADER_SUFFIX)) {
                ...
                // to only show personalized version rendered in browser
                if (r.settings.Rules.Rules && r.settings.Rules.Rules.length > 0) {
                    return createRendering(__assign(__assign({}, r), { componentName: __1.GLOBAL_LOADER }), placeholderKey, index, components, props, logger);
                }
            }
        }
    }
            ...
            if (html !== null) {
                if (typeof html === 'string' && !html.startsWith('<javascript-rendering ')) {
                    ...
                    if (!html.startsWith('"{') || !html.endsWith('}"')) {
                        return createRenderingFromHtml_1.createRenderingFromHtml(html, renderingContext, r, placeholderKey, index, components, config, logger);
                    }
                    ...
                    if (html && html.headHtml !== undefined && html.bodyHtml !== undefined) {
                        return (react_1.default.createElement(react_1.default.Fragment, null,
                            react_1.default.createElement(head_1.default, null, createRenderingFromHtml_1.createRenderingFromHtml(html.headHtml, renderingContext, r, placeholderKey, index, components, config, logger)),
                            createRenderingFromHtml_1.createRenderingFromHtml(html.bodyHtml, renderingContext, r, placeholderKey, index, components, config, logger)));
                    }
                }
            }
            else {
