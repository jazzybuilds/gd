...
function createRendering(r, placeholderKey, index, components, props, logger, options) {
    ...
    var reactRenderings = config.UNIFORM_REACT_RENDERINGS.split('|');
    var isReactRendering = reactRenderings.includes(componentName);
    ...
    logger.debug("- create rendering, name: " + r.componentName + ", id:" + r.renderingId + ", ds: " + ds);
    {
        ...
        if (htmlMap && componentName !== __1.GLOBAL_LOADER && !componentName.endsWith(__1.COMPONENT_LOADER_SUFFIX) && !isReactRendering) {
