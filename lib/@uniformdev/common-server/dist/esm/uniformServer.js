...
export function createUniformServer(logger, options) {
...
export function attachUniformServicesToServer(server, logger, options) {
    ...
    if (config.UNIFORM_MODE === UniformServerMode.preview) {
        server.get(['/uniform/api/service/status', '/uniform/api/deployment/status', '/uniform/api/deployment/status/:ticket'], function (_req, res) {
    ...
    else { // publish or mixed
        throw new Error("Not supported in v4-alpha");
    }
