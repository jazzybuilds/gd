            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 4:
                        ...
                        Status.writeStatus(outputDir, { status: "In progress", message: "Skip generating HTML site files (because no pages were changed since last publish)" });
                        ...
                        return [3 /*break*/, 9];
                    case 5:
                        Status.writeStatus(outputDir, { status: "In progress", message: "Generating HTML site files", total: paths.length + 2 }); // 404.html and 404/index.html
                        // the next export removes everything from the output folder so needs to be first
                    ...
                    case 11:
                        ...
                        Status.writeStatus(outputDir, { status: "In progress", message: "Creating .timestamp.txt file" });
                        logger.info("Creating .timestamp.txt file");
                        writeFileSync(outputDir + '/.timestamp.txt', JSON.stringify(timestamp));
                        logger.info("Starting deployment");
                        if (!(publishProvider.behavior !== 'replace-on-deploy')) return [3 /*break*/, 17];
                        Status.writeStatus(outputDir, { status: "In progress", message: "Uploading site assets separately" });
                        if (!existsSync(outputDir + '/static')) return [3 /*break*/, 14];
                        return [4 /*yield*/, publishProvider.deploy(outputDir + '/static')];
                    case 12:
                        _a.sent();
                        return [4 /*yield*/, del(outputDir + '/static')];
                    case 13:
                        _a.sent();
                        return [3 /*break*/, 17];
                    case 14:
                        if (!existsSync(outputDir + '/public')) return [3 /*break*/, 17];
                        return [4 /*yield*/, publishProvider.deploy(outputDir + '/public')];
                    case 15:
                        _a.sent();
                        return [4 /*yield*/, del(outputDir + '/public')];
                    case 16:
                        _a.sent();
                        _a.label = 17;
                    case 17:
                        Status.writeStatus(outputDir, { status: "In progress", message: "Uploading HTML site files" });
                        return [4 /*yield*/, publishProvider.deploy(outputDir)];
                    case 18:
                        publicUrl = _a.sent();
                        logger.info("Successfully deployed to " + publicUrl);
                        Status.writeStatus(outputDir, { status: "Success", message: publicUrl });
                        if (!(publishProvider !== undefined && !(publishProvider instanceof FakePublishProvider) && !config.UNIFORM_OPTIONS_DEBUG)) return [3 /*break*/, 20];
                        logger.info('Deleting temp dir: ' + outputDir);
                        return [4 /*yield*/, del([outputDir])];
                    case 19:
                        _a.sent();
