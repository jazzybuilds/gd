...
if (process.browser) {
	jQuery("body").on("click", ".js-accordion", function (e) {
		var _this = jQuery(this);
		e.preventDefault();
		var id = _this.parent().attr('id');
		var parent = jQuery('#'+id);
		var myRegexp = /(?:acc_1-0_panel_)([0-9]+)/;
		var match = myRegexp.exec(id);
	
		// run form validation, using sitecore validation
		var validator = $("main form");
		if(validator.valid()){
			if(parent.find('.postcode-lookup-step-1').is(':visible') && parent.find('.postcode-lookup-step-3').is(':hidden') ){
				parent.find('#PostcodeError').show();
				parent.find('#PostcodeLookupInput').addClass('input-validation-error').attr('aria-invalid', 'true').attr('aria-describedBy', 'PostcodeError');
			}else if(parent.find('#PostcodeAddressResultsSelect').is(':visible') && parent.find('#PostcodeAddressResultsSelect').val() === "" && parent.find('#HideEnterManuallyIfNotAvailable').is(':visible')){
				parent.find('#SelectAddressError').show();
				parent.find('#PostcodeAddressResultsSelect').addClass('input-validation-error').attr('aria-invalid', 'true').attr('aria-describedBy', 'SelectAddressError');
			}else {
				if(match != null && match.length > 1){
					_this.parent().prev().addClass('is-edited');
					jQuery('#acc_1-0_panel_'+(parseInt(match[1])+1)+'_trigger').click();
					setTimeout(function(){
						$('html, body').animate({
							scrollTop: (jQuery('#acc_1-0_panel_'+(parseInt(match[1])+1)).prev().offset().top-20)
						}, 1000);
					},100);
				}
			}
	...
	window.accordion = function (w, doc) {
		/**
		...
		var widgetTriggerClass = 'section-edit';
		...
		ARIAaccordion.create = function () {
			...
			for ( i = 0; i < widget.length; i++ ) {
				...
				if ( doc.querySelectorAll('#' + self.id + '> li').length ) {
					panels = doc.querySelectorAll('#' + self.id + ' li > ' + widgetPanel + ':not(.is-hidden)');
					headings = doc.querySelectorAll('#' + self.id + ' li > ' + widgetHeading + ':not(.is-hidden)');
				}
				else {
					panels = doc.querySelectorAll('#' + self.id + ' > ' + widgetPanel + ':not(.is-hidden)');
					headings = doc.querySelectorAll('#' + self.id + ' > ' + widgetHeading + ':not(.is-hidden)');
				}
